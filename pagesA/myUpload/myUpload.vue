<template>
	<view class="page-container my-upload">
		<NavBar :title="'美丽庭院'" :use-bg="true" :border="true" />
		<view class="hot-nav">
		...
		<view v-if="show === 0" class="show-content">
			<view v-for="(item, index) in list" :key="index" class="show-item">
				<view class="inside-border">
							...
							<swiper-item v-for="(e, i) in item.swiperList" :key="i">
								<view class="img">
									<van-image width="100%" height="100%" fit="cover"
										:src="e" />
										<view v-if="item.status === '已驳回'" class="status error">
											{{ item.status }}
										</view>
										<view v-else-if="item.status === '待审核' || item.status === '待处理'" class="status warning">
											{{ item.status }}
										</view>
					...
					<view v-if="item.status === '已驳回'" class="reject">
						驳回原因：{{ item.reject }}
					</view>
					<view v-if="item.status === '待审核'" class="cancel" @click.native.stop="cancel">
						取消
					</view>
				</view>
			</view>
		...
		<view v-if="show === 1" class="content">
			<view v-for="(item, index) in list" :key="index" class="item" @click="toDetail(item)">
				...
				<view v-if="item.status !== '待审核'" class="nav">
